...
def main():
    ...
    if args.distributed:
        if 'SLURM_NTASKS' in os.environ:
            args.rank, args.world_size, args.local_rank = distributed_utils.dist_init(
                port=args.port)
            args.num_gpu = 1
            args.device = 'cuda:%d' % args.local_rank
            args.world_size = torch.distributed.get_world_size()
            args.rank = torch.distributed.get_rank()
            _logger.info('[SLURM] Training in distributed mode. Process %d, local %d, total %d.' % (
            ...
            args.world_size = torch.distributed.get_world_size()
            args.rank = torch.distributed.get_rank()
            _logger.info('[TORCH] Training in distributed mode. Process %d, local %d, total %d.' % (
                args.rank, args.local_rank, args.world_size))
        else:
            _logger.error('Unsupported!')
            return
    assert args.rank >= 0
    ...
    data_config = resolve_data_config(
        vars(args), model=model, verbose=args.rank == 0)
    ...
    if args.data is not None:
        ...
        dataset_train = ListDataset(args.data_train_root,
                                    args.data_train_label)
        _logger.info('Loaded %d imgs from %s with ListDataset for train' %
                     (len(dataset_train), args.data_train_label))
    ...
    if args.data is not None:
        ...
        dataset_eval = ListDataset(args.data_val_root,
                                   args.data_val_label)
        _logger.info('Loaded %d imgs from %s with ListDataset for eval' %
                     (len(dataset_eval), args.data_val_label))
...
def validate(model, loader, loss_fn, args, amp_autocast=suppress, log_suffix=''):
    ...
    if args.rank == 0:
        _logger.info('[VAL] Acc@1: %.7f Acc@5: %.7f' %
